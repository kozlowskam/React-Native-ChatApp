...
class ChatRoom extends Component {
    ...
    return (
      <View ref={`msg${index}`} key={index} style={styles.message}>
        <Image
          style={styles.avatar}
          source={{
            uri: (author && author.gravatar) || "https://unsplash.it/50/50"
          }}
        />
        <View>
  ...
  render() {
    ...
    messageFormOptions.fields.text.onContentSizeChange = this.onContentSizeChange.bind(
      this
    );
    messageFormOptions.fields.text.stylesheet = {
      ...Form.stylesheet,
      textbox: { ...formStyles }
    };
...
const mapStateToProps = ({ user, loading, messages, users }) => ({
  user,
  loading,
  messages,
  users
...
export default connect(
  mapStateToProps,
  { loadUser, postMessage, subscribeToMessages, subscribeToUsers }
)(ChatRoom);
